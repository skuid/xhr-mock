 ...
 * @author Skuid
 */
MockXMLHttpRequest.addHandler = function(handler) {
  MockXMLHttpRequest.handlers.push(handler);
  return this;
};
...
/**
 * gets a handler with an appropriate route, without calling the handler
 * @param {MockXMLHttpRequest}
 * @returns {Handler|undefined}
 */
MockXMLHttpRequest.getHandler = function(request) {
  for (var i=0; i<MockXMLHttpRequest.handlers.length; ++i) {
    var currentHandler = MockXMLHttpRequest.handlers[i];
    if(currentHandler.match && currentHandler.match(request)) {
...
MockXMLHttpRequest.handle = function(request) {
  ...
  if(handler) return handler.callback(request, new MockResponse());
  return null;
};
...
function MockXMLHttpRequest() {
  ...
  // Stub out the upload functionality so that it at least won't error out
  this.upload = {
...
MockXMLHttpRequest.prototype.send = function(data) {
  var self = this,
    handler = MockXMLHttpRequest.getHandler(new MockRequest(self)),
    response;
  this.data = data;
  
  function applyResponse(self, response) {
